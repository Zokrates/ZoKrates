use crate::zir::result_folder::*;
use crate::zir::types::UBitwidth;
use crate::zir::{
    BooleanExpression, Expr, FieldElementExpression, Identifier, IfElse, IfElseExpression,
    IfElseOrExpression, Select, SelectExpression, SelectOrExpression, UExpression,
...
impl<'ast, T: Field> ResultFolder<'ast, T> for ZirPropagator<'ast, T> {
    ...
    >(
        ...
        let e = fold_if_else_expression(self, e)?;

        match e {
            IfElseOrExpression::IfElse(e) => match (*e.condition, *e.consequence, *e.alternative) {
                (_, consequence, alternative) if consequence == alternative => {
                    Ok(IfElseOrExpression::Expression(consequence.into_inner()))
                }
                (BooleanExpression::Value(true), consequence, _) => {
                    Ok(IfElseOrExpression::Expression(consequence.into_inner()))
                }
                (BooleanExpression::Value(false), _, alternative) => {
                    Ok(IfElseOrExpression::Expression(alternative.into_inner()))
                }
                (condition, consequence, alternative) => Ok(IfElseOrExpression::IfElse(
    ...
    fn fold_select_expression<E: Expr<'ast, T> + Select<'ast, T> + Clone + ResultFold<'ast, T>>(
        &mut self,
        e: SelectExpression<'ast, T, E>,
    ) -> Result<SelectOrExpression<'ast, T, E>, Self::Error> {
        let e = fold_select_expression(self, e)?;

        match e {
            SelectOrExpression::Select(e) => match e.index.clone().into_inner() {
                UExpressionInner::Value(v) => e
                    .array
                    .get(v as usize)
                    .ok_or_else(|| Error::OutOfBounds(v, e.array.len() as u128))
                    .map(|e| SelectOrExpression::Expression(e.clone().into_inner())),
                i => Ok(SelectOrExpression::Select(SelectExpression::new(
                    e.array,
...
mod tests {
    ...
    mod field {
        ...
        fn if_else() {
            ...
            assert_eq!(
                propagator.fold_field_expression(FieldElementExpression::if_else(
    ...
    mod bool {
        ...
        fn if_else() {
            ...
            assert_eq!(
                propagator.fold_boolean_expression(BooleanExpression::if_else(
                    BooleanExpression::Value(true),
                    BooleanExpression::Value(true),
                    BooleanExpression::Value(false)
                )),
