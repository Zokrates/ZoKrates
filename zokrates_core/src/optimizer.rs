            ...
            match *statement {
                ...
                FlatStatement::Definition(ref left, FlatExpression::Identifier(ref right)) => {
                    let r = match self.substitution.get(right) {
                        ...
                        None => format!("_{}", self.next_var_idx.increment()),
                    };
                ...
                FlatStatement::Definition(ref left, _) => {
                    self.substitution
                        .insert(left.clone(), format!("_{}", self.next_var_idx.increment()));
                }
                FlatStatement::Directive(ref d) => {
                    for o in d.outputs.iter() {
                        self.substitution
                            .insert(o.clone(), format!("_{}", self.next_var_idx.increment()));
                    }
                }
                _ => (),
            }
        ...
        let f: FlatFunction<FieldPrime> = FlatFunction {
            ...
            statements: vec![
                FlatStatement::Definition(
                    "b".to_string(),
                    FlatExpression::Identifier("a".to_string()),
                ),
                FlatStatement::Definition(
                    "c".to_string(),
                    FlatExpression::Identifier("b".to_string()),
                ),
    ...
    fn remove_multiple_synonyms() {
        let f: FlatFunction<FieldPrime> = FlatFunction {
            ...
            statements: vec![
                ...
                FlatStatement::Definition(
                    "e".to_string(),
                    FlatExpression::Identifier("d".to_string()),
                ),
                FlatStatement::Return(FlatExpressionList {
                    expressions: vec![
                        FlatExpression::Identifier("c".to_string()),
                        FlatExpression::Identifier("e".to_string()),
                    ],
                }),
            ],
            return_count: 2,
        ...
        let optimized: FlatFunction<FieldPrime> = FlatFunction {
            ...
            statements: vec![
                ...
                FlatStatement::Return(FlatExpressionList {
                    expressions: vec![
                        FlatExpression::Identifier("_0".to_string()),
                        FlatExpression::Identifier("_1".to_string()),
                    ],
                }),
            ],
            return_count: 2,
