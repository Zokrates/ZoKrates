...
pub fn compile<T: Field, R: BufRead, S: BufRead, E: Into<imports::Error>>(
    reader: &mut R,
    location: PathBuf,
    resolve_option: Option<fn(&PathBuf) -> Result<S, E>>,
    should_optimize: bool,
    should_include_gadgets: bool,
...
fn compile_aux<T: Field, R: BufRead, S: BufRead, E: Into<imports::Error>>(
    reader: &mut R,
    location: PathBuf,
    resolve_option: Option<fn(&PathBuf) -> Result<S, E>>,
    should_include_gadgets: bool,
                ...
                match resolve(&absolute_import_path) {
                    Ok(mut res) => {
                        let compiled = compile_aux(
                            &mut res,
                            absolute_import_path.parent().unwrap().to_path_buf(),
                            resolve_option,
...
mod test {
    ...
    fn no_resolver_with_imports() {
        ...
        let res: Result<FlatProg<FieldPrime>, CompileError<FieldPrime>> = compile(
            ...
            None::<fn(&PathBuf) -> Result<BufReader<Empty>, io::Error>>,
            false,
            false,
        );
        assert_eq!(
