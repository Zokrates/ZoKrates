...
impl<T: Field> Into<FlatFunction<T>> for R1CS {
    fn into(self: R1CS) -> FlatFunction<T> {
        ...
        let outputs: Vec<FlatExpression<T>> = self
            .outputs
            .iter()
            .map(|o| FlatExpression::Identifier(format!("inter{}", o)))
            .collect();
