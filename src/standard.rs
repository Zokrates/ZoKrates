...
pub struct R1CS {
	pub variable_count: usize, // # of elements in the witness in total
...
impl<T: Field> Into<FlatFunction<T>> for R1CS {
	fn into(self: R1CS) -> FlatFunction<T> {
        ...
        let variables = vec![0; self.variable_count].iter().enumerate().map(|(i, _)| format!("inter{}", i)).collect();
        ...
        statements.insert(0, FlatStatement::Directive(variables, inputs, Sha256Libsnark {}));
